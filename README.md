...
| `wn cleanup worktrees` | Remove git worktrees whose associated wn item is done and whose branch has been merged. Prompts for confirmation before removing. Use `--force` to skip the prompt; `--dry-run` to preview without removing; `--clean-ignored` to also remove gitignored files (build artifacts, temp config) before removal; `-b <ref>` to check against a specific ref instead of HEAD. |
