...
| `wn settings [--project]` | Open settings in `$EDITOR`. Default: user-level `~/.config/wn/settings.json`. Use `--project` to edit project-level `.wn/settings.json`, which overrides user settings when present. Set `"sort": "updated:desc,priority,tags"` for default list/fzf order. |
...
- **Default in settings** — In `~/.config/wn/settings.json` (or project-level `.wn/settings.json`; project overrides user), set `"sort": "updated:desc,priority"` (or any valid sort spec). This applies to `wn list` when `--sort` is not given, and to fzf/numbered lists for `wn pick`, `wn tag add -i`, `wn depend -i`, and `wn rmdepend -i`.
- **Cleanup defaults** — In settings (user or project `.wn/settings.json`), set `"cleanup": { "close_done_items_age": "30d" }` to configure the default age for `wn cleanup close-done-items` when `--age` is omitted. Durations accept `d` (days, 24h each), `h`, `m`, and `s` (e.g. `7d`, `48h`, `90m`).
