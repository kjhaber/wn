...
| `wn merge <branch> [-m "msg"]` | Squash-merge a feature branch into the current HEAD, record the commit hash as a `wn:commit` note, and mark the associated work item done. Opens `$EDITOR` for the commit message if `-m` is omitted. Use `--dry-run` to preview without making changes. Worktree-aware: git operations run in the main repo root. |
...
| `wn verify [--root]` | Run the shell command configured in `settings.verify` (e.g. `make all`, `npm test`). Useful for agents and humans to confirm the build passes. `--root` changes to the project root before running, useful when invoked from a git worktree. |
...
Tools: `wn_add`, `wn_list`, `wn_done`, `wn_undone`, `wn_desc`, `wn_show`, `wn_item`, `wn_claim`, `wn_release`, `wn_next`, `wn_pick`, `wn_root`, `wn_depend`, `wn_rmdepend`, `wn_note_search`, `wn_note_add`, `wn_note_edit`, `wn_note_rm`, `wn_duplicate`, `wn_prompt`, `wn_respond`, `wn_merge`, `wn_verify`. Use `wn_item` with a required id to get full item JSON and notes. For `wn_claim`, omit `for` to use default 1h so agents can renew without losing context. For `wn_next`, pass optional `tag` to return the next undone item with that tag, and optional `claim_for` to atomically claim it. For `wn_list`, pass `limit` and optional `offset` or `cursor` for a bounded window. For `wn_add`, pass optional `depends_on` (array of item IDs) to preserve queue order. Use `wn_note_search` to find items by note name/value (returns JSON array of `{id, description}`; supports `first`/`latest` to limit to one result). Use `wn_duplicate` to mark an item as a duplicate of another (sets status to closed, adds `duplicate-of` note). Use `wn_prompt` to create a blocking question for the user (adds a prompt item as a dep of the parent); use `wn_respond` to answer it and unblock the parent. Use `wn_merge` with `branch` and `message` to squash-merge a feature branch, record the commit hash, and mark the item done atomically; pass `dry_run: true` to preview. Use `wn_pick` with an item id to set it as the current task, or pass `"."` to select the item whose `wn:branch` note matches the current git branch. Use `wn_root` to get the main git repository root as JSON `{"root": "/absolute/path"}`; worktree-aware, resolves to the main repo even when called from a linked worktree. Use `wn_verify` to run the project's configured verify command (settings.verify) from the project root; returns JSON with stdout, stderr, and exit_code.
