...
| `wn cleanup worktrees` | Remove git worktrees whose associated wn item is done and whose branch has been merged. Use `--dry-run` to preview; `--clean-ignored` to also remove gitignored files (build artifacts, temp config) before removal; `-b <ref>` to check against a specific ref instead of HEAD. |
